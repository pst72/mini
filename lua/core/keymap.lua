return
{
vim.keymap.set({ 'v', 'n', 's' }, 'sw', '<cmd>Telescope grep_string pattern="<cword>"<cr>', { desc = 'Grep Word/s Under Cursor' }),
...
-- Resize with arrows
vim.keymap.set('n', '<C-Up>', ':resize -1<CR>', { desc = 'Resize Up' }),
vim.keymap.set('n', '<C-Down>', ':resize +5<CR>', { desc = 'Resize Down' }),
vim.keymap.set('n', '<C-Left>', ':vertical resize -1<CR>', { desc = 'Resize Left' }),
vim.keymap.set('n', '<C-Right>', ':vertical resize +5<CR>', { desc = 'Resize Right' }),
...
end)end, {desc = "[Mini.pick] PickTelescope ..."}),
...
vim.keymap.set('v', '<leader>hs', function() require('gitsigns').stage_hunk({ vim.fn.line('.'), vim.fn.line('v') }) end, { desc = 'Stage hunk (selected)' }),
vim.keymap.set('v', '<leader>hr', function() require('gitsigns').reset_hunk({ vim.fn.line('.'), vim.fn.line('v') }) end, { desc = 'Reset hunk (selected)' }),
...
vim.keymap.set('n', '<leader>gl', function() require('gitsigns').toggle_current_line_blame() end, { desc = 'Toggle Git blame line' }),
vim.keymap.set('n', '<leader>hd', function() require('gitsigns').diffthis() end, { desc = 'Git diff' }),
vim.keymap.set('n', '<leader>hD', function() require('gitsigns').diffthis('~') end, { desc = 'Git diff (HEAD)' }),
vim.keymap.set('n', '<leader>gd', function() require('gitsigns').toggle_deleted() end, { desc = 'Toggle Git deleted' }),

-- Git text object
vim.keymap.set({ 'o', 'x' }, 'ih', '<cmd>Gitsigns select_hunk<cr>', { desc = 'Select Git hunk' }),

-- telescope git pickers
vim.keymap.set('n', '<leader>gc', function() require('telescope.builtin').git_bcommits() end, { desc = 'Git commits (current buffer)' }),
vim.keymap.set('n', '<leader>gC', function() require('telescope.builtin').git_commits() end, { desc = 'Git commits (current directory)' }),
vim.keymap.set('v', '<leader>gC', function() require('telescope.builtin').git_bommits_range() end, { desc = 'Git commits (range)' }),
